...
import { groupedWineList } from '@/data/my-landing/wineCatalog';
...
export default function WineListSection() {
  ...
  const groupedShowcase = useMemo(
    () =>
      groupedWineList.map(([wineryName, wines]) => [
        wineryName,
        wines
          .map((wine) => ({
            source: wine,
    ...
    <>
      <section className="mx-auto w-full max-w-[1120px] px-6 py-16 sm:px-8 lg:px-12 lg:py-20">
            ...
            <div className="mt-3 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-end">
              <div>
                ...
                <p className="mt-4 max-w-[64ch] text-[14px] leading-[1.95] text-[rgba(31,27,22,0.76)]">
                  FINDESTが現在ご提案しているワイン一覧です。料理との相性や提案シーンを想い描きやすいよう、レストランの現場に寄り添うかたちで整えています。
                </p>
              </div>
              <div className="rounded-[20px] border border-[rgba(31,27,22,0.08)] bg-[rgba(255,255,255,0.52)] px-5 py-4 backdrop-blur-[2px]">
                ...
                <p className="mt-2 text-[12.5px] leading-[1.85] text-[rgba(31,27,22,0.72)]">
                  産地やヴィンテージの背景よりも、まずは今の飲み頃とレストランでの使いやすさが伝わる構成を大切にしています。
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-12 space-y-16">
          {groupedShowcase.map(([wineryName, wines], groupIndex) => (
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => setActiveWineId(source.id)}
                          className="mt-3.5 flex flex-1 flex-col text-left focus:outline-none"
                          aria-label={`${showcase.displayTitle} の詳細を見る`}
                        >
                          ...
                          <div className="mt-3 space-y-1.5 text-[12px] leading-[1.7] text-[rgba(31,27,22,0.72)]">
                            <p>{showcase.regionVillage}</p>
                            <p>{showcase.vintage}</p>
                            <p>{showcase.typeGrapeLine}</p>
                          </div>

                          <p className="mt-3 min-h-[4.8em] text-[12.5px] leading-[1.78] text-[rgba(31,27,22,0.84)]">
                            {showcase.shortLine}
                          </p>
